        ...
        if (file.type && file.type !== 'application/json' && !file.name.endsWith('.json')) {
            ...
            reader.onload = function(e) {
                try {
                    ...
                    try {
                        // Attendre que le module CommentaryGenerator soit chargé
                        if (window.CommentaryGenerator && typeof window.CommentaryGenerator.enrichPlays === 'function') {
                            console.log('Enrichissement des commentaires en cours...');
                            jsonData = window.CommentaryGenerator.enrichPlays(rawJsonData);
                            console.log('Enrichissement terminé !');
                        } else {
                            console.warn('Générateur de commentaires non disponible');
                        }
                    } catch (enrichError) {
...
function updatePossession(row) {
    ...
    if (row['commentaire-Situation'] && (row['commentaire-Situation'].toLowerCase() === "possession" || 
                                        row['commentaire-Situation'].toLowerCase() === "shoot")) {
    ...
    // Mise à jour de la balle dans les commentaires
    updateCommentaryBall(possession);
}
...
function updatePossessionIndicator(possession) {
    ...
    // Réinitialiser les indicateurs de possession
    if (teamABox) teamABox.classList.remove('has-possession');
    if (teamBBox) teamBBox.classList.remove('has-possession');
    ...
    // Récupérer les noms des équipes depuis les sélecteurs
    let teamAName = 'EQUIPE A';
    let teamBName = 'EQUIPE B';
...
/**
 * Met à jour la balle dans la section de commentaires
 * @param {string} possession - Nom de l'équipe qui a la possession
 */
function updateCommentaryBall(possession) {
    // Récupérer les balles de commentaire
    const commentaryBallA = document.getElementById('commentary-ball-a');
    const commentaryBallB = document.getElementById('commentary-ball-b');
    
    // Masquer les deux balles d'abord
    if (commentaryBallA) commentaryBallA.style.display = 'none';
    if (commentaryBallB) commentaryBallB.style.display = 'none';
    
    // Si aucune possession, on ne montre pas de balle
    ...
    if (teamASelect && teamASelect.value) teamAName = teamASelect.value;
    if (teamBSelect && teamBSelect.value) teamBName = teamBSelect.value;
    
    // Afficher la balle du côté approprié
    if (possession === teamAName && commentaryBallA) {
        commentaryBallA.style.display = 'block';
    } else if (possession === teamBName && commentaryBallB) {
        commentaryBallB.style.display = 'block';
    }
}

/**
 * Met à jour les commentaires avec les données de la ligne en utilisant le générateur avancé
 * @param {Object} row - Données de la ligne
 */
function updateCommentary(row) {
    // Récupérer les éléments du DOM pour les commentaires
    const actionPlayerElement = document.querySelector('.action-player');
    const actionTypeElement = document.querySelector('.action-type');
    const actionResultElement = document.querySelector('.action-result');
    const currentActionElement = document.querySelector('.current-action');

    // Générer et afficher le commentaire enrichi

    // Affichage du pop-up de points si tir réussi
    const popupResult = (row.actionResult || row['commentaire-Succes'] || '').toLowerCase();
    const popupAction = (row.actionType || row['commentaire-Situation'] || '').toLowerCase();
    if ((popupResult === 'success' || popupResult.includes('réussi')) && (popupAction === '2pt' || popupAction === '3pt' || popupAction === 'shoot' || popupAction.includes('tir') || popupAction.includes('shoot'))) {
        // Détecter le type de tir (2PT ou 3PT)
        let shotType = null;
        if (window.CommentaryGenerator && typeof window.CommentaryGenerator.detectShotType === 'function') {
            shotType = window.CommentaryGenerator.detectShotType(row);
        } else if (typeof detectShotType === 'function') {
            shotType = detectShotType(row);
        }
        if (shotType === '2PT' || shotType === '3PT') {
            // Import dynamique pour compatibilité non-module
            if (window.showPointsPopup) {
                window.showPointsPopup(shotType);
            } else {
                import('./points-popup.js').then(mod => {
                    if (mod.showPointsPopup) mod.showPointsPopup(shotType);
                }).catch(()=>{});
            }
        }
    }
    if (window.CommentaryGenerator && typeof window.CommentaryGenerator.generateCommentary === 'function') {
        const enrichedComment = window.CommentaryGenerator.generateCommentary(row);
        if (currentActionElement) {
            currentActionElement.innerHTML = enrichedComment;
            // Dynamique : couleur selon le résultat
            currentActionElement.classList.remove('success', 'failure', 'neutral');
            const result = (row.actionResult || '').toLowerCase();
            if (result === 'success') {
                currentActionElement.classList.add('success');
            } else if (result === 'fail' || result === 'failure') {
                currentActionElement.classList.add('failure');
            } else {
                currentActionElement.classList.add('neutral');
            }
        }
        console.log('[updateCommentary] Commentaire enrichi:', enrichedComment);
    }
    
    if (!actionPlayerElement || !actionTypeElement || !actionResultElement) {
        console.warn('Éléments de commentaire non trouvés dans le DOM');
        return;
    }
    
    // Vérifie si le générateur de commentaires est disponible
    if (window.CommentaryGenerator && typeof window.CommentaryGenerator.generateCommentary === 'function') {
        // Générer un commentaire avancé
        const generatedComment = window.CommentaryGenerator.generateCommentary(row);
        
        if (generatedComment) {
            // Afficher le commentaire généré comme HTML formaté
            if (generatedComment && currentActionElement) {
                currentActionElement.innerHTML = generatedComment;
                // Animation pour nouvelle action (optionnel)
                currentActionElement.classList.remove('show');
                void currentActionElement.offsetWidth;
                currentActionElement.classList.add('show');
                console.log(`Commentaire avancé: ${generatedComment}`);
                return;
            }
        }
    }
    
    // Fallback : Utiliser le mécanisme de base si le générateur n'est pas disponible
    const player = row['commentaire-Joueur'] || '';
    const action = row['commentaire-Situation'] || 'Action';
    const team = row['commentaire-Equipe'] || '';
    const result = row['commentaire-Succes'] || '';
    
    // Mise à jour du joueur (avec l'équipe entre parenthèses si disponible)
    actionPlayerElement.textContent = player ? 
    ...
    actionPlayerElement.classList.remove('team-a', 'team-b');
    
    // Appliquer la classe selon l'équipe
    if (team === teamAName) {
        actionPlayerElement.classList.add('team-a');
    } else if (team === teamBName) {
        actionPlayerElement.classList.add('team-b');
    }
    
    // Mise à jour du type d'action
    actionTypeElement.textContent = action;
    
    // Mise à jour du résultat avec classe CSS appropriée
    actionResultElement.textContent = result || '—';
    
    // Appliquer la classe CSS selon le résultat
    actionResultElement.classList.remove('success', 'failure');
    if (result && result.toLowerCase().includes('réussi')) {
        actionResultElement.classList.add('success');
    } else if (result && result.toLowerCase().includes('raté')) {
        actionResultElement.classList.add('failure');
    }
    
    console.log(`Commentaire de base: ${player} - ${action} - ${result}`);
}
    ...
    for (let playerNum = 1; playerNum <= 5; playerNum++) {
        ...
        for (const statType of statTypes) {
            ...
            if (row[idStat] !== undefined) {
                ...
                const currentValue = parseFloat(row[idStat]) || 0;
                
                // Récupérer la valeur précédente si elle existe
                const previousValue = parseFloat(previousStatsValues[idStat]) || 0;
                
                // Format intelligent: entier si pas de décimale, sinon afficher la décimale
                statElement.textContent = currentValue % 1 === 0 ? Math.floor(currentValue) : currentValue.toFixed(1);
