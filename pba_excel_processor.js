  ...
  function workbookToJson(workbook) {
    try {
      ...
      for (let rowNumber = 2; rowNumber <= worksheet.rowCount; rowNumber++) {
        ...
        const rowDict = {};
        let hasContent = false; // Indicateur pour vérifier si la ligne contient des données
        ...
        row.eachCell({ includeEmpty: false }, function(cell, colNumber) {
          if (colNumber <= headers.length) {
            ...
            if (cellValue !== null && cellValue !== undefined) {
              ...
              // Vérifier si la cellule a un contenu significatif
        ...
        // Vérifier si au moins une cellule importante contient une valeur
        if (!hasContent) {
          // Vérifier rapidement les colonnes importantes (comme quarter, time, etc.)
          const importantColumns = ['scoreboard-QT', 'scoreboard-Temps', 'commentaire-Situation'];
          for (const col of importantColumns) {
            const normalizedCol = normalizeCssId(col);
            if (normalizedCol in rowDict && rowDict[normalizedCol] !== "") {
              hasContent = true;
              break;
            }
          }
        ...
        // N'ajouter la ligne que si elle contient des données significatives
        if (hasContent) {
